...
public class MergeController : SingletonScene<MergeController>
{
    ...
    public (Vector3, MergePoint) GetPosition(BlockColor blockColor, IMovable movable, Blcok.IDisposable d)
    {
    ...
    private void SetPoint(int index, BlockColor blockColor, IMovable movable, Blcok.IDisposable d)
    {
    ...
    public void MergeToColor(BlockColor color)
    {
        ...
        for (int k = firstIndex; k < firstIndex + 3; k++)
        {
            ...
            uiText += point.gameObject.name + "\n"; 
            //CoroutineTestUI.Instance.SetText($"{uiText} {k - firstIndex + 1}");
            point.Movable.JumpAndMerge(k - firstIndex + 1, targetMergePos,CallBakc);
            try
            {
                RemoveBlock();
            }
            catch (Exception ex)
            {
                ...
                //CoroutineTestUI.Instance.SetText($"Ошибка при DestroyBlock:");
            }
