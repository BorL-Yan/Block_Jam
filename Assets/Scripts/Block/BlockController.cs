...
using Game;
using UnityEngine;
using IDisposable = Blcok.IDisposable;
...
public class BlockController : MonoBehaviour, IMovable, ISelect, IDisposable
{
    ...
    public void JumpAndMerge(int index, Vector3 position, Action callback)
    {
        _movment.Jump(index, position, DestroyBlock, callback);
    } 
